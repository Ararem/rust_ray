...
target!(UI_PERFRAME_SPAMMY, r"High-frequency (i.e. every frame) logs from the UI");
target!(PROGRAM_MESSAGE_POLL_SPAMMY, r"High-frequency (i.e. every frame) logs from the program message poll loop");
target!(PROGRAM_MESSAGE_POLL, r"Logs from the program message poll loop that shouldn't be called super often, or spam the log");
